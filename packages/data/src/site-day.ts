...
export function siteDayBounds(
  ...
  // Reconstruct "midnight in tz" for TODAY and for the NEXT civil day.
  // Using the next day's midnight - 1ms correctly handles DST transitions
  // (a spring-forward day is 23h; a fall-back day is 25h).
  const start = localMidnight(year, month, day, tz)

  // Advance by one civil day (compute next day's midnight in local tz)
  ...
  // The next calendar day in local tz (guaranteed to be tomorrow)
