...
- **Custom brand pages** (`/sites`, track 023): a per-site **Custom brand** toggle
  (`setCustomBrand` → `Site.customBrandEnabled`). This is the LIVE half of a two-gate design —
  a per-site React module in `apps/user/brands` decides whether a bespoke page EXISTS, this
  column decides whether it SERVES. Separating them lets a merged page sit dark until the
  customer approves it, and a broken one be pulled without a revert and a deploy. Switching it
