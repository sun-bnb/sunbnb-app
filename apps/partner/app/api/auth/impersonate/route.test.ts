...
// Stub next-auth so importing AuthError doesn't pull in the runtime env loader,
// which fails in vitest due to a "next/server" ESM resolution quirk.
vi.mock('next-auth', () => {
...
describe('GET /api/auth/impersonate', () => {
  ...
  it('calls signIn with the impersonation provider and the supplied token', async () => {
    // signIn never returns normally on success — simulate by throwing a
    // Next.js redirect marker we can detect.
    ...
    try {
      ...
      // If we reach here, signIn didn't throw — that's a test failure since
      // success is expected to bubble NEXT_REDIRECT.
