...
function MollieStatusCard({ mollieStatus }: { mollieStatus: MollieStatus }) {
  ...
  const statusConfig: Record<string, { bg: string; text: string; labelKey: string }> = {
    'completed':  { bg: 'bg-emerald-50', text: 'text-emerald-700', labelKey: 'onboardingVerified' },
    'in-review':  { bg: 'bg-amber-50',   text: 'text-amber-700',   labelKey: 'onboardingInReview' },
    'needs-data': { bg: 'bg-orange-50',  text: 'text-orange-700',  labelKey: 'onboardingNeedsData' },
  }

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold text-gray-900">{t('molliePayments')}</h2>
        <Link
          href="/account/mollie"
        ...
        {mollieStatus.isConnected ? (
          <>
            ...
            {mollieStatus.onboardingStatus && (() => {
              const cfg = statusConfig[mollieStatus.onboardingStatus] ?? { bg: 'bg-gray-50', text: 'text-gray-600', labelKey: null }
              return (
                <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${cfg.bg} ${cfg.text}`}>
                  {cfg.labelKey ? t(cfg.labelKey as any) : mollieStatus.onboardingStatus}
                </span>
              )
            })()}
          </>
      ...
      <form action={formAction}>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field name="firstName" label={t('firstName')} placeholder="John" defaultValue={account.firstName || ''} required />
            <Field name="lastName" label={t('lastName')} placeholder="Doe" defaultValue={account.lastName || ''} required />
            <Field name="email" label={t('email')} type="email" placeholder="john@company.com" defaultValue={account.email || ''} required />
            <Field name="phoneNumber" label={t('phoneNumber')} type="tel" placeholder="+358 40 123 4567" defaultValue={account.phoneNumber || ''} required />
          </div>
        </div>

        {/* Company info */}
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
          <h2 className="text-sm font-semibold text-gray-900 mb-4">{t('company')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field name="company" label={t('companyName')} placeholder="Sunbnb Oy" defaultValue={account.company || ''} required />
            <Field name="businessId" label={t('businessId')} placeholder="FI12345678" defaultValue={account.businessId || ''} required />
            <div className="md:col-span-2">
              <Field name="websiteUrl" label={t('website')} type="url" placeholder="https://sunbnb.com" defaultValue={account.websiteUrl || ''} />
            </div>
            <div className="md:col-span-2">
              <Field name="address" label={t('streetAddress')} placeholder="Paseo Marítimo 12" defaultValue={account.address || ''} required />
            </div>
            <Field name="postalCode" label={t('postalCode')} placeholder="29602" defaultValue={account.postalCode || ''} required />
            <Field name="city" label={t('city')} placeholder="Marbella" defaultValue={account.city || ''} required />
            <div className="md:col-span-2">
              <SelectField name="country" label={t('country')} defaultValue={account.country || ''} required options={COUNTRY_OPTIONS} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
          <h2 className="text-sm font-semibold text-gray-900 mb-4">{t('billing')}</h2>
          <div className="grid grid-cols-1 gap-4">
            <Field name="bankAccount" label={t('iban')} placeholder="FI12 3456 7891 2345" defaultValue={account.bankAccount || ''} />
          </div>
        </div>
