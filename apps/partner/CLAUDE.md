...
- **Seat ids in the inventory editor** (track 021): the editor renders `{parcel}-{row}-{seq}-{member}` (`1-1-1-2`) via `formatSeatId` (`@repo/data/seat-label`), which UNPACKS the stored `seatLabel` — storage packs row and unit ordinal into one segment (`1-101-2`), hiding the unit address that a device is assigned to. **Display only**: `seatLabel` is untouched, so devices, printed QR cards and anything painted still resolve as before. `qr-print-button.tsx` deliberately still prints the stored form — it is the one place a label leaves the screen and lands on a bed, so changing it would desync new cards from ones already stuck on. Parsing rather than shipping the unit's `parcel`/`row`/`seq` columns keeps the C2 payload cut intact (the label already carries all four numbers). Pinned by a test that unpacks every generated label back to the independently-computed unit address, so the id on screen and the address a device answers for cannot drift.
