...
describe('POST /api/auth/forgot-password', () => {
  ...
  it('returns 400 when email exceeds 320 chars', async () => {
    const request = makeRequest({ email: 'a'.repeat(310) + '@example.com' })
    const response = await POST(request)
    expect(response.status).toBe(400)
    const data = await response.json()
  ...
  it('returns 400 when email has no domain TLD', async () => {
    const request = makeRequest({ email: 'user@nodomain' })
    const response = await POST(request)
    expect(response.status).toBe(400)
    const data = await response.json()
    expect(data.error).toContain('Invalid email')
  ...
  it('returns 400 when email has no @ symbol', async () => {
    const request = makeRequest({ email: 'notanemail' })
    const response = await POST(request)
    expect(response.status).toBe(400)
    const data = await response.json()
    expect(data.error).toContain('Invalid email')
