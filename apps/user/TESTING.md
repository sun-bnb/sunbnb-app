...
- `app/api/hw/[code]/state/route.test.ts` — HW device state route: the Q9 client filter (scanner UAs curl/requests/browser declined, empty UA declined, CONTAINS-match so a firmware version bump still passes, unknown code asserted **byte-identical** to a filtered request, neither reaching the DB, `HW_CLIENT_UA`-unset fails CLOSED ⇒ 503), binding resolution from the `Device` table (unknown code, `retired` declined, `provisioned` served, empty binding declined, lookup failure ⇒ 503, normalised code asserted on the query) + mount-order emission, address resolution (the exact 4-part `siteId_parcel_row_seq` key asserted — dropping `row` would silently serve a neighbouring row's unit, since `seq` repeats in every row; no unit at the address ⇒ non-200), one case per compound state → wire state, the read-only today-row rule (a multiday guest checked in yesterday reads RESERVED, not OCCUPIED), aggregate rule, fail-safe (unrecognised state ⇒ OCCUPIED; DB throw ⇒ 503), ETag/304, and the poll cadence (the resolved preference is served rather than a hardcoded interval, a cadence change busts the ETag so a 304 cannot hide it, and the reads go through the cached accessor), **power mode** (the resolved mode is served beside the cadence, every mode round-trips, a mode change busts the ETag so a 304 cannot hide it, a cadence the mode cannot keep is fitted to the band before it reaches a potted device, and an unreadable mode falls back to `deep_sleep` rather than continuous), and **tracking on the poll** (wire v2: the `x-sunbnb-telemetry` header is recorded on the `Device` row; an UNASSIGNED device is recorded before its 401; an unknown code with a partner claim registers through `applyDeviceClaim`; a fresh unchanged row is NOT written and the claim rides the same throttle; a moved `loc` writes; a header-less poll past the floor stamps only `lastSeenAt`; a failed write still 200s; filtered-out traffic is never recorded) (76 tests). Filter extracted to `hw/[code]/hw-filter.ts` (`screenDeviceRequest` + `normalizeCode`), shared with the legacy telemetry POST
- `app/api/hw/[code]/telemetry/route.test.ts` — HW telemetry stub (P1.5): client-filter parity with the state route (scanner/empty UAs declined, CONTAINS-match, unknown code byte-identical to a filtered request, `HW_CLIENT_UA`-unset ⇒ 503 fail-closed), and a `204` that never depends on the body (well-formed, malformed non-JSON, empty, and unknown-field payloads all accepted), code normalisation before the binding lookup, retired-device decline, last-values persistence through the shared recorder — UNTHROTTLED even on a fresh unchanged row, partner claim applied from the header, omit-not-null field semantics, DB failure still `204` (19 tests)
- `app/api/hw/[code]/device-report.test.ts` — the self-report that rides the poll (wire v2): `parseTelemetryHeader` (every documented key, missing/empty header ⇒ `{}`, malformed pair skipped without dropping the rest, unknown key ignored, whitespace tolerated, empty value omitted, string caps at column width), `reportFromBody` (legacy JSON → same shape, wrong types dropped), `reportIsDue` (the throttle: fresh + unchanged ⇒ no write; due on the 5-min floor, never-seen row, fw change, running-location change, battery ≥50 mV / RSSI ≥6 dB exactly at the threshold and not below, first battery/RSSI reading, uptime DROP but not rise), `reportData` (omit-not-null, every field + timestamp) (21 tests)
