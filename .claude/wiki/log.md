...
## [2026-08-20] ingest | Short address-keyed QR entry (track 022 P1–P4)
- mode: patch (entities/reservation.md) + planned-note (index.md)
- changed: `.claude/wiki/entities/reservation.md` (the POS/QR row of § Creation paths — the canonical entry is now `/q/[site]/[unit]`, the old `sites/[id]/pos/...` redirects to it, and the URL is keyed by site code + unit ADDRESS rather than by ids), `.claude/wiki/index.md` (Planned subsystems: `physical-identity`, with an explicit hold).
- reason: track 022 shipped the guest-facing QR entry point (uncommitted at time of writing): `Site.code` (`S-` + 6 Crockford), `parseUnitAddress`, `/q/{siteCode}/{parcel}-{row}-{seq}`, legacy routes redirecting, and the printed card emitting the short URL. The one claim in the wiki that this made misleading was the creation-paths row, which would have sent an agent to build against the legacy route. Everything else that changed fails the gate: the length/QR-version work is performance-shaped, the loaders are a refactor, and the new tests guard existing behaviour.
- deliberately NOT done: no new page. The genuinely load-bearing new rule — a card binds to the SPOT, not to a seat row — belongs with the external-code family (`Device.code` / `PartnerAccount.code` / `Site.code`) and the shared unit address, which now has two consumers (the HW state route and the QR route) and has therefore earned a page under the two-strikes rule. But [[track:021]] is mid-expand and production still lacks `SunbedGroup.parcel`/`row_idx`, so a page written today would document a moving model; it is queued under Planned with that hold recorded instead.
- `last_verified` on entities/reservation.md deliberately NOT bumped: only the row above was re-checked against code, and the page's remaining claims (schema, partner paths) were not. Bumping would have claimed a freshness this pass did not earn. Noted while there: invariant 6 still describes transitions as "use the helpers in manage/actions.ts" with no mention of the track-018 machine — not false (the helpers are still the entry points and raw status writes are still forbidden), but thin; left for a pass that verifies the whole page.
- by: claude (opus-5)

## 2026-08-29 — canonical-layer compaction (context budget)
- mode: split (new subsystems/migrations.md) + index entry
- changed: new `.claude/wiki/subsystems/migrations.md` (environment topology + laptop-run sequence, moved verbatim out of `.claude/rules/migrations.md`, which breached its 80-line hard cap at 114); `index.md` gains the row.
- reason: the auto-loaded layer had reached **137 KB per session**. Root `CLAUDE.md` `@`-imported all five app/package files (~97 KB) regardless of task, and the `/update-knowledge` size budgets were measured in LINES — so `apps/user/CLAUDE.md` passed at 175/300 lines while being 34.8 KB, its longest single line a 2,296-byte table cell. Budgets are now in BYTES with a 300 B per-line cap; root is a router; per-app detail moved to on-demand siblings (`TESTING.md` ×3, `packages/data/REFERENCE.md`) mirroring the existing `UI.md` pattern. Result: 137 KB → 35.4 KB always-loaded.
- deliberately NOT done: the moved bulk went to app-local siblings, NOT into the wiki. `.claude/wiki/README.md` § What the wiki does NOT contain explicitly excludes duplicated CLAUDE.md content — the wiki summarises and indexes, canonical statements stay in the CLAUDE tree. Only the migrations *narrative* qualified as subsystem material. No `last_verified` bumps: nothing was re-verified against code, only relocated.
- by: claude (opus-5)
