...
- **Schema & migrations.** Schema lives only in `packages/data`. Prefer additive (nullable/defaulted) columns over destructive ones (expand/contract). Follow the sequence: `migrate:local` → integration tests → `migrate:test` → notify apps; deploy via the migrate-before-deploy scripts. Never edit an applied migration — fix forward. A schema or payment-core change is *always* an architecture pass (see `migrations.md`, `data-access.md`, `payments.md`).
...
- **Fee cascade & VAT.** Reuse the three-tier cascade and reverse-VAT helpers in `@repo/data`; never recompute money inline. The consumer always pays the listed price only — the commission is deducted from partner revenue (applicationFee + PLATFORM invoice), never added to the consumer total, uniformly across reservations, F&B orders, and dine-in tabs (see `payments.md`).
